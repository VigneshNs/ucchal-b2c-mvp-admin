...
@Component({
  ...
  styleUrls: ['./add-super-category.component.css']
})
export class AddSuperCategoryComponent implements OnInit {
...
uploadImages(id) {
 ...
 this.categoryService.uploadImages(this.imageHolder, id).subscribe(data => {
   this.updateSuperCategoryImagesName(this.imageHolder.categoryImage, id);
  /*  this.superCategoryFilter = data;
   this.superCategoryData = new MatTableDataSource<PeriodicElement>(data); */
   this.urls = [];
 }, error => {
...
updateSuperCategoryImagesName(name, id) {
 this.superCategoryModel = new SuperCategory();
