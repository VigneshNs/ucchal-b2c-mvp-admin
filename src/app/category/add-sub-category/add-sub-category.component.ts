...
export class AddSubCategoryComponent implements OnInit {
  ...
  displayedColumns: string[] = ['imageTab', 'categoryName', 'description', 'metatag', 'delete'];
  fileToUpload;
  imageError: boolean;
  urls: any[];
  reader: FileReader;
  ...
  selectDropdown(e, ix) {
    ...
    const controlAttributValue = (
      <FormArray> this.subCategoryForm.controls
 ['attribute']).at(ix).get('fieldValue') as FormArray;
    const control =
    (<FormArray>this.subCategoryForm.controls['attribute']).at(ix).get('fieldType') as FormArray;
...
return s; */
  ...
  addSubCategory() {
    if (this.headerCatSelectedData && this.selectedMainCategory && this.selectedStatus) {
      ...
      this.subCategoryModel.status = this.selectedStatus;
...
this.categoryService.addSubCategory(this.subCategoryModel, this.headerCatSelectedData, this.selectedMainCategory).subscribe(data => {
  ...
  uploadImages(supId, minId, id) {
    ...
    this.categoryService.uploadSubCategoryImages(formData, id).subscribe(data => {
      this.updateSubCategoryImagesName(data, supId, minId, id);
  ...
  updateSubCategoryImagesName(imagedata, supId, minId, id) {
    this.subCategoryModel = new SubCategory();
    this.subCategoryModel.subCategoryImageName = imagedata.originalname;
