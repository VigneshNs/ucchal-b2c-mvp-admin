...
@Component({
  selector: 'app-edit-fifth-row',
  templateUrl: './edit-fifth-row.component.html',
  styleUrls: ['./edit-fifth-row.component.css']
})
export class EditFifthRowComponent implements OnInit {
  ...
  getFifthDetails() {
    this.cmsSerivce.getSingleFifthProducts(this.id).subscribe(data => {
      console.log(data);
  ...
  onSubmit() {
    /* console.log(this.selection); */
    this.message = 'Fifth Row Created';
    this.rowModel = new FifthRowModel();
    const array: any = [];
    ...
    this.cmsSerivce.addFifthRow(this.rowModel).subscribe(data => {
      this.snackBar.open(this.message, this.action, {
        duration: 2000,
      });
      this.router.navigate(['cms/viewFifth']);
    }, err => {
      console.log(err);
    });
  }
