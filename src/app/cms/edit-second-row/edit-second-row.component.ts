...
@Component({
  selector: 'app-edit-second-row',
  templateUrl: './edit-second-row.component.html',
  styleUrls: ['./edit-second-row.component.css']
})
export class EditSecondRowComponent implements OnInit {
  ...
  getSecondDetails() {
    this.cmsSerivce.getSingleSecondProducts(this.id).subscribe(data => {
      console.log(data);
  ...
  onSubmit() {
    console.log(this.selection);
    this.message = 'Second Row Created';
    this.rowModel = new SecondRowModel();
    const array: any = [];
    ...
    this.cmsSerivce.addSecondRow(this.rowModel).subscribe(data => {
      this.snackBar.open(this.message, this.action, {
        duration: 2000,
      });
      this.router.navigate(['cms/viewSecond']);
    }, err => {
      console.log(err);
    });
  }
