...
@Component({
  selector: 'app-edit-sixth-row',
  templateUrl: './edit-sixth-row.component.html',
  styleUrls: ['./edit-sixth-row.component.css']
})
export class EditSixthRowComponent implements OnInit {
  ...
  getSixthDetails() {
    this.cmsSerivce.getSingleSixthProducts(this.id).subscribe(data => {
      console.log(data);
  ...
  onSubmit() {
    console.log(this.selection);
    this.message = 'Sixth Row Created';
    this.rowModel = new SixthRowModel();
    const array: any = [];
    ...
    this.cmsSerivce.addSixthRow(this.rowModel).subscribe(data => {
      this.snackBar.open(this.message, this.action, {
        duration: 2000,
      });
      this.router.navigate(['cms/viewSixth']);
    }, err => {
      console.log(err);
    });
  }
