...
import { DiscountFilter } from './../../../shared/model/discount-filter.model';
...
export class DiscountSettingComponent implements OnInit {
  ...
  discountFilterModelData: DiscountFilter;
  discountFilterModel: DiscountFilter;
...
settingsModel: ProductSettings;
  ...
  createForm() {
    this.productSettingsForm = this.fb.group({
      discountRangeMinimum: [],
      discountRangeMaximum: [],
      color: [],
      material: [],
      occasion: [''],
      size: [''],
  ...
  getProductSettings() {
    this.productService.getProductSettings().subscribe(data => {
      this.discountFilterModelData = data;
  ...
  saveDiscountRange() {
    ...
    this.discountFilterModel = new DiscountFilter();
    this.discountFilterModel.minDiscount = this.productSettingsForm.controls.discountRangeMinimum.value;
    this.discountFilterModel.maxDiscount = this.productSettingsForm.controls.discountRangeMaximum.value;
    this.productService.addDiscountRange(this.discountFilterModel).subscribe(data => {
      this.discountFilterModelData = data;
  ...
  deleteDiscountRange(id, val) {
    this.message = 'Price Range Deleted';
    this.productService.deleteDiscountRange(id, val).subscribe(data => {
      this.discountFilterModelData = data;
      this.snackBar.open(this.message, this.action, {
        duration: 3000,
      });
  ...
  openDeleteDiscountRange(id, val):void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if(result) {
        ...
        this. deleteDiscountRange(id, val);
      }
    });
  }
  deleteColor(val) {
