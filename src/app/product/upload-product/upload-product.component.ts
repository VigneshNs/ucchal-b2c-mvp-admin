 ...
 */
  ...
  categorySubCategory(e) {
    ...
    const findColor = this.subCategoryDetails.attribute.find(fi => (fi.fieldSetting === 'Color'));
    const findSize = this.subCategoryDetails.attribute.find(fi => (fi.fieldSetting === 'Size'));
    this.dataSubColor = this.subCategoryDetails.attribute.find(fi => (fi.fieldSetting === 'Color'))  === undefined ?   this.emptySubDeatils : findColor.fieldValue;
    this.dataSubSize = this.subCategoryDetails.attribute.find(fi => (fi.fieldSetting === 'Size'))  === undefined ?  this.emptySubDeatils :  findSize.fieldValue;
    for (const ite of this.dataSubDeatils) {
      if (!dataSub[ite.fieldName]) {
        const subele = ite.fieldName;
        dataSub[subele] = '';
  ...
  onUpload() {
    ...
    reader.onload = (e) => {
      ...
      if (skuProduct.length !== checkSKU.length && this.isValidate === false) {
        this.isCatalogueSKU = true;
        this.loader = true;
      }
      if (this.vendorSku.length !== 0 && this.isCatalogueSKU === false  && this.isValidate === false) {
       // this.vendorSku.forEach(vendor => {
         // parent.forEach(product => {
           // if (vendor.sku === product.sku) {
             // this.isSameSKU = true;
              //this.loader = true;
            //}
          //});
        //});
        parent.forEach(product => {
          if (this.vendorSku.find(vendor => vendor.sku === product.sku)) {
            this.isSameSKU = true;
            this.loader = true;
            this.errorSku = product.sku;
          }
        });
      }
      console.log(this.isSameSKU);
      if (this.isValidate === false && this.isCatalogueSKU === false && this.isSameSKU === false) {
      ...
      this.productService.uploadProduct(mainArray).subscribe(v1 => {
        console.log(v1);
        this.imageUpdate();
        /* this.loader = true;
